...
double f(int i, int u, int p, int ok) { // i = atual[0:N-1], u = ultimo[a:b], p = penultimo[a:b]
    ...
    if (i == N) {
        ...
        ok = ok + (turnPenultimoUltimoPrimeiro == giro) + (turnoUltimoPrimeiroSegundo == giro);
    ...
    for (int at = 0; at < 2; at++) // escolher i do set A ou B
    {
        ...
        if (turn == 0 || turn == giro) {
            r = min(r, TRIAREA(M[pivot][0], M[u][i - 1], M[at][i]) + f(i + 1, at, u, ok + (turn == giro)));
        }
    }
    return r;
}
void init_dp() { rep(i, 0, N + 1) rep(a, 0, 2) rep(b, 0, 2) rep(c, 0, 2) dp[i][a][b][c] = -1; }
